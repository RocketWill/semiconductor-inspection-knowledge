...
This note follows a p-type MOS capacitor from accumulation through depletion and inversion, then connects the changing surface state with the measured C–V response. Flat-band voltage may shift even when the device geometry stays the same because work-function difference and charge near the oxide act like an additional gate bias. Under the example conditions, a short calculation gives \(C'_{FB}\approx371\ \mathrm{nF/cm^2}\). The project link is intentionally limited: my previous optical systems did not measure C–V curves. What carries over is the need to store configuration and measurement context with the result.
...
![小黑轉動閘極電壓曲柄，使 P 型 MOS 電容依序進入累積、空乏與反轉](../assets/02-semiconductor-characterization-illustrations/05-mos-surface-states.png)

> 圖 1：作者依個人課程筆記設計並重新整理；負閘極電壓使電洞在界面累積，正閘極電壓先將電洞推離表面，再於較高電壓下吸引電子形成反轉層。圖中的紅色標記提醒空乏寬度是在矽內增加。
...
| 符號 | 意義 |
| --- | --- |
| \(C'_{ox}\) | 單位面積氧化層電容，常用 \(\mathrm{F/cm^2}\) |
...
$$
C_{ox}=\frac{\varepsilon_{ox}A}{t_{ox}}=C'_{ox}A
$$

這裡最常出現的不是推導問題，而是單位問題。\(C'_{ox}\) 和 \(C_{ox}\) 差了一個面積，數值不能直接混用。若厚度以 nm 給出、介電常數使用 \(\mathrm{F/cm}\)，也必須先把厚度換成 cm。

從公式可以直接看出，氧化層越薄，\(C'_{ox}\) 越大。不過這只是在幾何模型中的方向，不能據此說氧化層越薄就沒有代價；漏電、崩潰與製程變異仍是另外的問題。
...
當正閘極電壓使 \(W\) 增加時，\(C'_s\) 下降，串聯後的總電容也跟著下降。這條關係讓 C–V 曲線不再只是形狀記憶：曲線向下，其實對應到電場進入矽內更深。
...
在低頻或接近準靜態的條件下，少數載子有時間生成、復合並回應訊號，反轉層可以參與小訊號變化，量測電容可能再次上升。
...
![小黑調整節拍器，低頻時少數載子能跟上，高頻時則來不及回應](../assets/02-semiconductor-characterization-illustrations/06-cv-frequency-response.png)

> 圖 2：作者依個人課程筆記設計並重新整理；由累積進入空乏時電容下降，反轉區則會因量測頻率而出現不同響應。藍線表示少數載子能跟隨的低頻情況，紅線表示高頻下電容維持在較低值。
...
## 8. Flat-band capacitance 為什麼不是 \(C_{ox}\)
...
C'_{FB}
=
\frac{C'_{ox}C'_{s,FB}}
{C'_{ox}+C'_{s,FB}}
$$

所以 \(C'_{FB}\) 會小於 \(C'_{ox}\)。如果直接把兩者視為相同，從 C–V 曲線讀出的 \(V_{FB}\) 位置也會跟著偏掉。
...
$$
C'_{ox}\approx690.6\ \mathrm{nF/cm^2}
$$
...
## 10. A Small Project Connection: Configuration Is Part of the Result

The measurement conditions in the C–V section reminded me of the Vision Console.

When I built that system, I stored exposure, gain, ROI, camera selection, and processing presets because they affected what the inspection pipeline received. Saving only the final defect result made later comparison much harder (which was easy to forget once the interface looked complete).

The connection to MOS measurement is limited, but useful. A C–V curve also needs its frequency, temperature, device area, sweep direction, and measurement setup. Change those conditions and the curve may change. Its interpretation certainly can.

The Vision Console did not measure capacitance, and an optical preset cannot explain oxide charge. But the shared lesson is simple: configuration is part of the result, not a note added after storage.

## 11. 寫在公式旁邊的檢查

這一篇整理完後，公式旁可以留下幾個簡單提醒：

- 空乏區在矽內，不在氧化層；
- \(C'_{ox}\) 是單位面積電容，\(C_{ox}\) 才是總電容；
- 串聯後的 \(C'\) 不應大於任一個分電容；
- flat band 不一定發生在 \(V_G=0\)；
- C–V 曲線一定要附上頻率與掃描條件；
- curve shift 是證據，不是氧化層缺陷類型的完成診斷。

和前一篇二極體 I–V 類似，真正需要保留的不是一條單獨的曲線，而是曲線、元件條件、量測設定和使用模型之間的關係。只是到了 MOS 電容，頻率與界面狀態變得更難忽略。

## Current Scope

At this stage, the path from gate voltage to surface charge, and then to a C–V curve, is reasonably clear. That part is clearer now. Oxide and flat-band capacitance can also be calculated under a stated set of assumptions and used to estimate \(V_{FB}\).

One shifted or stretched curve is still not a complete defect diagnosis. Separating fixed oxide charge from mobile ions, trapped charge, and interface states requires more careful measurement design and fitting than this note covers. For now, the useful boundary is knowing which assumptions were made and which information must stay attached to the curve.
...
1. *E2 Vision Console*, a multi-camera inspection and configuration system I developed. It is referenced here only for measurement-configuration traceability, not as an electrical characterization instrument.
