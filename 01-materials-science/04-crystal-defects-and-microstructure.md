...
我原本比較容易把「缺陷」理解成材料中不應該存在的錯誤。整理擴散與差排的內容後，我才逐漸分清楚：缺陷本身不一定代表材料失效。空位提供替位型原子移動的位置，差排讓金屬能在合理的應力下產生塑性變形，受控制的摻雜原子則是半導體功能的一部分。真正需要判斷的是缺陷的類型、濃度、位置和尺度是否符合製程與使用需求。
...
結果表示溫度從 $500\ \mathrm K$ 提高到 $1000\ \mathrm K$ 時，平衡空位比例可能增加約五個數量級。這個例子讓我更容易理解為什麼熱處理溫度的改變不能只看成「加熱得更快」：缺陷數量與擴散能力本身也會隨溫度產生非常大的變化。
...
若 $Q_d$ 使用單一原子的 $\mathrm{eV/atom}$，分母應搭配 $k_{\mathrm B}T$；若 $Q_d$ 使用 $\mathrm{J/mol}$，則搭配 $RT$。這是我在計算時最需要先檢查的地方，因為公式形式看起來相同，但能量尺度與常數不能混用。
...
雖然溫度只提高 $200^\circ\mathrm C$，擴散係數卻增加約三個數量級。計算完成後，我會先檢查趨勢：溫度升高時 $D$ 應該變大；若結果相反，通常需要重新確認溫度倒數的順序或負號。
...
整理到這裡時，我認為「微觀組織」和「晶體結構」最容易混淆。晶體結構描述原子如何週期排列，例如 FCC 或 BCC；微觀組織則描述晶粒尺寸、形狀、取向、相分布與缺陷配置。兩種材料可以具有相同晶體結構，卻因微觀組織不同而呈現不同強度、擴散或破壞行為。
...
因此我會把檢測結果當成縮小假設範圍的起點，再依照問題選擇驗證方式：
...
假設晶圓上出現固定方向的細長亮線，我不會直接把它命名為差排，而會先檢查：
