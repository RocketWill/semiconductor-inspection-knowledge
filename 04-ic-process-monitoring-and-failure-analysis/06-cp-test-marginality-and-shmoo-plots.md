...
> 圖 1：CP tester 經 probe card、probe needle 與 pad 接觸 DUT，再取得量測 response；連接方式與 pad 數量為概念示意。
...
> 圖 2：Input transition 後，output 需要一段時間進入穩定區；sampling point 太接近 transition region 時，判讀風險增加。
...
假設測試程式在指定 voltage、frequency、sampling time 與 temperature 下執行，最後得到 Pass。這個結果能證明 DUT 在當次條件下通過，卻沒有告訴後續判斷條件稍微改變後會發生什麼。
...
> 圖 3：概念性的 voltage–frequency Shmoo map；每個方格代表一組測試條件，僅用來表示 Pass、marginal 與 Fail region。
